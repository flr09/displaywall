...
- `software/` -> Enthält alle Downloads, Installations-Skripte und Docker-Konfigurationen, die offline auf die Pis übertragen werden.
  - `setup.sh` -> Shell-Skript zur Installation von Docker und Snap (Xibo-Player).
...
- [ ] Erstellung einer Anleitung für die "Client-Pis" (die beiden anderen Raspberry Pis), welche nur den Player, aber nicht das CMS hosten.
